...
contract SimpleStorage {
    ...
    function addFile(string memory _ipfsHash, string memory _studentName, string memory _studentHobby) public {
        ...
        userFiles[msg.sender].push(File(userFileCount[msg.sender], _ipfsHash, _studentName, _studentHobby));
    ...
    function getFile(address _user, uint _fileId) public view returns (string memory ipfsHash, string memory studentName, string memory studentHobby) {
        require(_fileId > 0 && _fileId <= userFileCount[_user], "File ID out of range.");
        File memory file = userFiles[_user][_fileId - 1];
        return (file.ipfsHash, file.studentName, file.studentHobby);
    }
